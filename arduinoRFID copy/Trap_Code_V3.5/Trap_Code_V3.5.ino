...
// void selectedDogsRFID(int scannedTag) {
...
void setInTrap(int scannedTag) {
  ...
  for (JsonPair keyValue : doc.as<JsonObject>()) {
    ...
    int rfid = (int)keyValue.value()["rfid"];
    Serial.println("Name:"+ name);
    Serial.println("rfid:"+ rfid);

    if (rfid == scannedTag) 
      fb.setBool("/dogs/" + name +"/inTrap", true);
  }
}
...
void printText(String text) {
  unsigned long currMill = millis();
  Serial.println(currMill);
  Serial.println(currMill+500);
  lcd.setCursor(0, 1);//shows the current text
  lcd.print(text);
  prevMill=currMill;
  blank=false;
